Вижу проблему в логах сборки. Ошибка в строке 49 с выполнением скрипта `/app/diagnose.sh` - получаем ошибку "not found" (код выхода 127).

Проблема скорее всего в том, что скрипт создается с символами переноса строки `\n` в виде текста, а не как реальные переносы строк. Давайте исправим это, разделив создание диагностического скрипта на несколько команд:
...
Эта версия Dockerfile должна решить проблему с выполнением скрипта, так как теперь каждая строка скрипта добавляется отдельно, без использования `\n`, что гарантирует правильное форматирование. Также улучшено создание других скриптов аналогичным образом.