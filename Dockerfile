...
# Добавляем дополнительную проверку и используем npm run build вместо npx vite build
RUN echo "Запуск сборки через npm script..."
RUN npm run build || (echo "Ошибка сборки через npm script" && exit 1)
RUN echo "Проверка содержимого директории после сборки:"
RUN ls -la
# Сохраняем исходную команду для совместимости
...
COPY --from=build /app/dist/ /usr/share/nginx/html/ || echo "Предупреждение: проблема при копировании файлов"
